...
async function saveApiKey() {
    ...
    if (!key) {
        showAlert('createEventAlert', 'Please enter an API key', 'error');
        return;
    }
                ...
                <div class="event-card ${isPast ? 'past' : ''}">
                    ...
                    ${event.roles && event.roles.length > 0 ? `
                        <div class="event-roles">
                            ${event.roles.map(r => `<div class="role-badge">${r.emoji || '👤'} ${r.name}</div>`).join('')}
                        </div>
                    ` : ''}
                ...
                <div class="preset-card">
                    ...
                    ${preset.roles && preset.roles.length > 0 ? `
                        <div class="preset-roles">
                            ${preset.roles.map(r => `<div class="role-badge">${r.emoji || '👤'} ${r.name} (${r.maxSlots || '∞'})</div>`).join('')}
                        </div>
                    ` : ''}
                    <div class="preset-actions">
                        <button class="btn btn-secondary" onclick="usePreset('${key}')">
                            ✨ Use
                        </button>
                        <button class="btn btn-danger" onclick="deletePreset('${key}')">
                            🗑️ Delete
                        </button>
                    </div>
                </div>
