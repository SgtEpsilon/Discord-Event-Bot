...
async function loadEvents() {
    try {
        ...
        document.getElementById('eventsContainer').innerHTML = `<div class="alert alert-error">❌ Error loading events: ${escapeHtml(error.message)}</div>`;
    }
}
...
async function loadPresets() {
    try {
        ...
        if (Object.keys(presets).length === 0) {
            container.innerHTML = `<div class="loading">No presets found. Create your first preset!</div>`;
            return;
        }
        ...
        document.getElementById('presetsContainer').innerHTML = `<div class="alert alert-error">❌ Error loading presets: ${escapeHtml(error.message)}</div>`;
    }
}
...
async function deleteEvent(eventId) {
    ...
    try {
        await apiRequest(`/api/events/${encodeURIComponent(eventId)}`, { method: 'DELETE' });
        loadEvents();
        loadDashboardData();
    } catch (error) {
...
// ==========================================
// NOTIFICATION CHANNEL SAVE FUNCTION
// ==========================================
async function saveNotificationChannel() {
    ...
    try {
        // Call backend API to save notification channel
        await apiRequest(`/api/notification-channel/${STATE.currentGuildId}`, {
            method: 'POST',
            body: JSON.stringify({
                channelId
            })
        ...
        showAlert('notificationStatus', `❌ Error: ${error.message}`, 'error');
    }
}
